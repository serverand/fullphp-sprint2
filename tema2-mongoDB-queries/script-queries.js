...
// nota 09: No aconsegueixo que el primer document amb score=131 no surti.
// segons la documentació tindria que ser així: db.restaurants.find( { "grades.score": { $elemMatch: { $gt: 80, $lt: 100 } } } )
// però això no retorna res.
...
db.restaurants.find({ $and: [{"address.coord.1" : { $lt : -65.754168 }}, {"grades.score" : { $gt : 70 }},  {"cuisine" : { $not: /American/}}] })
...
db.restaurants.find({"address.coord.1" : { $lt : -65.754168 }, "grades.score" : { $gt : 70 },  "cuisine" : { $not: /American/}  })
...
db.restaurants.find({"grades[1].grade" : "A", "grades[1].score" : 9, "grades[1].date": "2014-08-11T00:00:00Z"},{"restaurant_id":1,"name":1,"grades":1})
...
db.restaurants.find({$and : [{"address.coord.1" : {$gt : 42}},{"address.coord.1" : {$lt : 52}}]},{"restaurant_id":1,"name":1,"address.coord":1})
...
db.restaurants.find({$and : [{"address.coord.0": { $regex: /./}},{"address.coord.1": { $regex: /./}} ]})
...
db.restaurants.find({"grades.score" : { $mod: [ 7, 0 ] }},{"restaurant_id":1,"name":1,"address.grade":1})
...
db.restaurants.find({"name" : { $regex: /mon/ }},{"name":1,"borough":1,"grades.coord.0":1,"grades.coord.1":1,"cuisine":1})
...
db.restaurants.find({"name" : { $regex: /^Mad/ }},{"name":1,"borough":1,"grades.coord.0":1,"grades.coord.1":1,"cuisine":1})